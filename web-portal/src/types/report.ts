...
export const REPORT_STATUSES = ['PENDING', 'CONFIRMED', 'REJECTED', 'RESOLVED'] as const;
export type ReportStatus = (typeof REPORT_STATUSES)[number];

// ── Detection Source (mirrors DetectionSource.kt) ─────────────

export const DETECTION_SOURCES = ['CV_ONLY', 'SENSOR_ONLY', 'DUAL_CONFIRMED', 'MANUAL'] as const;
export type DetectionSource = (typeof DETECTION_SOURCES)[number];

// ── Damage Types ──────────────────────────────────────────────

export const DAMAGE_TYPES = ['pothole', 'bump', 'speed_bump', 'roughness'] as const;
...
export function getSeverityLevel(severity: number): SeverityLevel {
  ...
  if (severity < 0.50) return 'medium';
