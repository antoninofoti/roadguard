...
function TrendChart({
  ...
  const yMean =
    h -
    pad -
    (points.reduce((s, p) => s + p.count, 0) / points.length / max) *
      (h - 2 * pad);
  const x0 = pad,
    x1 = w - pad;
  ...
  return (
    <svg
      ...
      aria-label="Monthly report trend chart"
    >
      {/* Grid lines */}
      ...
      <line
        x1={x0}
        y1={Math.max(pad, Math.min(h - pad, y0))}
        x2={x1}
        y2={Math.max(pad, Math.min(h - pad, y1))}
...
function SummaryCard({ label, value, sub, Icon, accent }: SummaryCardProps) {
  return (
    <motion.div
      ...
      className="bg-surface-800 border border-glass-border rounded-xl p-4 flex items-start gap-3"
    >
      <div
...
export function AnalyticsPage() {
  ...
  const allFilters = useMemo(
    () => ({
      ...DEFAULT_FILTERS,
      statuses: ["PENDING", "CONFIRMED", "REJECTED", "RESOLVED"] as Array<
        (typeof import("../types/report").REPORT_STATUSES)[number]
      >,
    }),
    [],
  );
  const { reports, isLoading } = useReports(
    allFilters as Parameters<typeof useReports>[0],
  );
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Data-driven maintenance intelligence · {reports.length} reports
              indexed
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <section aria-label="Summary statistics">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                  ...
                  <SummaryCard
                    ...
                    value={`${summary.dualConfirmedPct}%`}
              ...
              <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Condition Trend */}
                <div className="lg:col-span-1 bg-surface-800 border border-glass-border rounded-xl p-5">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${trend.color}`}
                    >
                  ...
                  <TrendChart
                    points={forecast.monthly}
                    slope={forecast.slope}
                  />
                  <div className="space-y-2 mt-4 pt-3 border-t border-glass-border">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500">Regression Slope</span>
                      <span className="text-white font-mono">
                        {forecast.slope.toFixed(3)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-white font-bold">
                        {forecast.nextMonthPredicted} reports
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="lg:col-span-2 bg-surface-800 border border-glass-border rounded-xl p-5 relative overflow-hidden group">
              ...
              <section aria-label="Priority zones and damage clusters">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {/* Priority Zones */}
                  <div className="bg-surface-800 border border-glass-border rounded-xl p-5">
                    <div className="flex items-center gap-2 mb-4">
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <span
                                  className={`text-xs font-bold shrink-0 ${priorityColor(z.priorityScore)}`}
                                >
                                  {z.priorityScore.toFixed(3)}
                                </span>
                              </div>
                              {/* Priority bar */}
                              <div className="mt-1 h-1 rounded-full bg-surface-700">
                                <div
                      ...
                      <div className="space-y-3">
                        {clusters.map((c, i) => (
                          <motion.div
                            ...
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ delay: i * 0.05 }}
                            className="bg-surface-700/50 rounded-lg p-3 border border-glass-border"
                          >
                            ...
                            <div className="flex gap-3 mt-2 text-xs text-slate-500">
                              ...
                              <span>
                                {c.centroidLat.toFixed(4)}°,{" "}
                                {c.centroidLng.toFixed(4)}°
                              </span>
                            </div>
                          </motion.div>
                        ))}
